		...
		<div class="n_viewform">
			...
			<div class="comments">	
			    ...
			    <div class="comment-input">
				    ...
				    <button id="submitComment" value="${param.qna_seq}" data-logged-in="${isLoggedIn}" >댓글<br>등록</button>
			    </div>
			    </c:if>
			    <c:if test="${empty user.us_id}">
				<div class="comment-input">
				    <label><textarea id="commentContent" placeholder="로그인후 입력이 가능합니다." disabled></textarea></label>
				    <button id="submitComment" value="${param.qna_seq}" data-logged-in="${isLoggedIn}" disabled >댓글<br>등록</button>
			    </div>
				</c:if>
